...
**Correct answer for PT migration:** Move `lib/mcp/alphaclaw-mcp.js` and `lib/agents/` wholesale into PT. In PT they call AlphaClaw via its HTTP API / CLI, not via internal requires. The stdio MCP server process is then managed by PT, not AlphaClaw. AlphaClaw stays clean.
...
Day 2: implement full adapter CLI+HTTP + wire all MCP tools + write unit tests + build UTS orchestration skeleton + run first E2E scenario.
Day 3: harden retry/backoff + write remaining E2E + file AlphaClaw cleanup PR + finalize all docs + get CI green on both new repos.
...
- 1. `agentic-stack` → portable `.agent/` folder across IDEs as our default baseline, align all code converge conventions
...
`AlphaClaw` (infrastructure / managed dependency) → `PT` (middleware / adapters / tooling) → `UTS` (application / orchestration / workflows). This is a classic layered architecture and it's correct. The alternative — keeping everything in one repo — produces an unmaintainable monolith that can't track upstream at all.

### 4. "First principles + design doc before code" is the right call given the complexity

The plan explicitly says: draft adapter API and types first, get a design doc reviewed, then implement. Given that PT and UTS don't exist yet, this prevents building on unstable foundations. It will save 3x the time in avoided rewrites compared to just starting to code.
...
The client we built (Ollama → GLM-5.1:cloud fallback → qwen3.5-local, LM Studio OpenAI-compat) is the right abstraction: a unified `LocalAgentClient` that dispatches to available backends. Migrating it to PT is a cut-and-paste — the design doesn't change, only where it lives. The plan correctly identifies PT as the owner of this code.
...
2. **Replace the 3-day timeline with milestone gates:**
   ...
   - Gate 3: First UTS orchestration flow runs end-to-end

3. **Remove `lib/mcp/` and `lib/agents/` from AlphaClaw** before cherry-picking to pr-4-macos. Flag as PT-destined scaffolding in the commit message.
...
5. **Pick one external reference repo** (recommendation: agentic-stack for the `.agent/` portable folder concept) and evaluate the other two after the adapter is working.

6. **Defer public NPM publish decision** but document the intended package names now: `@perpetua-tools/alphaclaw-adapter`, `@orama-system/orchestrator`. Reserve the npm scopes.
