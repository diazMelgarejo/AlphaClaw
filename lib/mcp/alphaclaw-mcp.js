 ...
 * Xcode 26.3 BETA / Claude Code can manage AlphaClaw directly.
 *
 * Transport: stdio (local integration, no network auth needed)
 * Register:  claude mcp add --transport stdio alphaclaw -- node lib/mcp/alphaclaw-mcp.js
 *
 * Tools:
...
// ─── Project root (two levels up from lib/mcp/) ──────────────────────────────
...
function getOrchestrator() {
  if (!_orchestrator) {
    try {
      _orchestrator = require("../agents/orchestrator");
    } catch (e) {
