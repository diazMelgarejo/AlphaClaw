...
const normalizeOnboardingModels = (models) => {
  ...
  const providerOrder = new Map(
    kOnboardingProviderOrder.map((provider, index) => [provider, index]),
  );
  return Array.from(deduped.values()).sort((a, b) => {
    const aRank = providerOrder.has(a.provider) ? providerOrder.get(a.provider) : 999;
    const bRank = providerOrder.has(b.provider) ? providerOrder.get(b.provider) : 999;
