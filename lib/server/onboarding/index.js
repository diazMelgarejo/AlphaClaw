...
const createOnboardingService = ({
  ...
  const completeOnboarding = async ({
    ...
    const {
      ...
      selectedProvider,
      hasCodexOauth,
      readOnlyMode: validatedReadOnlyMode,
    } = validation.data;

    const hasGithubRepo = !!githubRepoInput;
    const repoUrl = hasGithubRepo ? resolveGithubRepoUrl(githubRepoInput) : "";
    const remoteUrl = repoUrl ? `https://github.com/${repoUrl}.git` : "";
    const configPath = `${OPENCLAW_DIR}/openclaw.json`;
    const existingConfigPresent =
      (importMode || validatedReadOnlyMode) && fs.existsSync(configPath);
    if (validatedReadOnlyMode && !existingConfigPresent) {
      return {
        status: 400,
        body: {
          ok: false,
          error: "Read-only mode requires an existing OpenClaw config",
        },
      };
    }
    ...
    if (importMode && existingConfigPresent && !validatedReadOnlyMode) {
      const systemVars =
    ...
    const hadImportedGit =
      !validatedReadOnlyMode && importMode && fs.existsSync(`${OPENCLAW_DIR}/.git`);
    if (hadImportedGit) {
      try {
    ...
    if (!validatedReadOnlyMode && remoteUrl && (hadImportedGit || !fs.existsSync(`${OPENCLAW_DIR}/.git`))) {
      await shellCmd(
    ...
    } else if (!validatedReadOnlyMode && importMode && remoteUrl) {
    ...
    if (!validatedReadOnlyMode && !fs.existsSync(`${OPENCLAW_DIR}/.gitignore`)) {
      fs.copyFileSync(
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          onboarded: true,
          reason: validatedReadOnlyMode
            ? "read_only_complete"
            : importMode
              ? "import_complete"
              : "onboarding_complete",
          readOnly: validatedReadOnlyMode,
    ...
    if (!validatedReadOnlyMode && githubToken && repoUrl) {
      try {
