...
const { isReadOnlyOnboarding } = require("./onboarding/state");
...
const isOnboarded = () => fs.existsSync(kOnboardingMarkerPath);
...
const ensureGatewayProxyConfig = (origin) => {
  ...
  if (isReadOnlyOnboarding({ fsModule: fs })) return false;
  try {
    const configPath = `${OPENCLAW_DIR}/openclaw.json`;
    const cfg = JSON.parse(fs.readFileSync(configPath, "utf8"));
