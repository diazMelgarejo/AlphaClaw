...
const { isReadOnlyOnboarding } = require("./server/onboarding/state");
const {
...
registerSystemRoutes({
  ...
  kOnboardingMarkerPath: constants.kOnboardingMarkerPath,
  kControlUiSkillPath: constants.kControlUiSkillPath,
...
const doSyncPromptFiles = () => {
  if (isReadOnlyOnboarding({ fsModule: fs })) return;
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  if (isOnboarded()) {
    ...
    if (!isReadOnlyOnboarding({ fsModule: fs })) {
