...
export const WelcomePreStep = ({ onSelectFlow }) => {
  return html`
    <div class="space-y-3">
      ...
      <button
        type="button"
        onclick=${() => onSelectFlow(kGithubFlowReadOnly)}
        class="w-full flex items-center gap-4 text-left p-4 rounded-xl ac-path-card"
      >
        <div
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5">
            ...
            <span class="shrink-0 ml-1 text-[11px] text-cyan-300">
              Read-only
            </span>
          </div>
          <div
          ...
          >
            Attach AlphaClaw to the local OpenClaw config without rewriting it.
          </div>
        </div>
      </button>
    </div>
