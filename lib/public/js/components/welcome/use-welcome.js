...
export const useWelcome = ({ onComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    const preflightError = (() => {
      ...
      const isReadOnlyFlow = githubFlow === kGithubFlowReadOnly;
      if (
        !isReadOnlyFlow &&
        (!normalizedVals.MODEL_KEY ||
          !String(normalizedVals.MODEL_KEY).includes("/"))
      ) {
      ...
      if (
        ...
        return 'Target repo must be in "owner/repo" format, or leave both GitHub fields blank.';
      }
      if (
    ...
    const githubFlow = normalizedVals._GITHUB_FLOW || kGithubFlowFresh;
    const wasImport = githubFlow === kGithubFlowImport;
    const wasReadOnly = githubFlow === kGithubFlowReadOnly;
    try {
      const result = await runOnboard(vars, normalizedVals.MODEL_KEY, {
        importMode: wasImport,
        readOnlyMode: wasReadOnly,
      });
