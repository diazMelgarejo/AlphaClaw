...
const kFileDraftStorageKeyPrefix = "alphaclaw.browse.draft.";
const kLegacyFileDraftStorageKeyPrefix = "alphaclawBrowseDraft:";
const kDraftIndexStorageKey = "alphaclaw.draftIndex";
const kDraftIndexChangedEventName = "alphaclaw:browse-draft-index-changed";
...
const getFileDraftStorageKey = (filePath, useLegacyPrefix = false) =>
  `${useLegacyPrefix ? kLegacyFileDraftStorageKeyPrefix : kFileDraftStorageKeyPrefix}${String(filePath || "").trim()}`;

const readStoredFileDraft = (filePath) => {
  try {
    if (!filePath) return "";
    const draft =
      window.localStorage.getItem(getFileDraftStorageKey(filePath)) ||
      window.localStorage.getItem(getFileDraftStorageKey(filePath, true));
...
const writeStoredFileDraft = (filePath, content) => {
  try {
    if (!filePath) return;
    window.localStorage.setItem(
      getFileDraftStorageKey(filePath),
      String(content || ""),
    );
  } catch {}
};

const clearStoredFileDraft = (filePath) => {
  try {
    if (!filePath) return;
    window.localStorage.removeItem(getFileDraftStorageKey(filePath));
    window.localStorage.removeItem(getFileDraftStorageKey(filePath, true));
  } catch {}
};

const readDraftIndex = () => {
  try {
    const rawValue = window.localStorage.getItem(kDraftIndexStorageKey);
...
const writeDraftIndex = (draftPaths) => {
  try {
...
const updateDraftIndex = (filePath, hasDraft) => {
  if (!filePath) return;
  const normalizedPath = String(filePath || "").trim();
  if (!normalizedPath) return;
  const nextDraftPaths = readDraftIndex();
  if (hasDraft) nextDraftPaths.add(normalizedPath);
  else nextDraftPaths.delete(normalizedPath);
  writeDraftIndex(nextDraftPaths);
};
