...
const stringifyWatchdogValue = (value) => {
...
export const formatWatchdogCopyAllText = ({
  ...
  sections.push(stringifyWatchdogValue(watchdogStatus || {}));
  ...
  if (!Array.isArray(events) || events.length === 0) {
    ...
    sections.push(
      events
        .map((event, index) => {
          const createdAt = event?.createdAt || "unknown";
          const eventType = event?.eventType || "event";
          const status = event?.status || "unknown";
          const source = event?.source || "unknown";
          const details = stringifyWatchdogValue(event?.details || {});
          return [
            `### Incident ${index + 1}`,
